...
eq..  obj=e=w1 * (D * sum(k, (z(k) + c(k)) * qprime(k) + (A(k) + S(k)) * b(k) + 1/2 * (hb/D + h(k)/P(k)) * qprime(k) * q(k)) - Z1star)/Z1star + w2 * (Z2star - sum(k,1 - e(k)))/Z2star;

model case1 /all/;
solve case1 using minlp min obj;
display obj.l, y.l, q.l, qprime.l, b.l, t.l, U.l, V.l, e.l;
